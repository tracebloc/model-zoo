...
and infers a well-formed payload, with a PER-TEMPLATE report (the acceptance sweep).

    python tools/od_acceptance_sweep.py --steps 8 --experiments 2 \
        --out sweep.json --markdown sweep.md

WHAT THIS IS, AND WHAT IT IS NOT (the acceptance sweep's own framing)
-------------------------------------------------------------
`e2e-test-agent`'s `harness/od/probes.py` is a STRUCTURAL probe
...
It is NOT a constant in this file. (internal ref) is fixing the from-scratch
...
RESOLUTION IS RECORDED, NOT ASSUMED
--------------------------------------------------
...
def schema_path() -> Path:
    ...
    ``hf_transformer`` with (internal ref)), and pinning a filename breaks on the
...
def seeding_index() -> Dict[str, str]:
    ...
    the hosting decision has not hosted the dumps. `EXPECTS_SEED` therefore means "a
    ...
    for key, record in dump_survey(REPO_ROOT).items():
        ...
        if status == EXPECTS_SEED:
            index[stem] = "scratch (seed declared, not hosted -- the hosting decision)"
        elif status == NO_SEED:
...
def make_batch(torch, image_size: int, num_classes: int):
    ...
    torchvision's background index before the model sees it, so a
...
def observed_input_shape(torch, model, images, targets) -> Optional[List[int]]:
    ...
    the (internal ref) point. Recorded per template so a cost figure can never be
...
def _quality_cause(n_preds: Optional[Sequence[int]], seeded: bool) -> str:
    ...
    Derived, never a constant -- an internal ticket's normalisation fix is expected to
    ...
    *,
    ...
    for path in selected:
        ...
        row: Dict[str, Any] = {
            ...
            "seeded": False,  # the hosting decision: no OD seed is hosted, so never True
        }
...
def markdown(report: Dict[str, Any]) -> str:
    ...
    out.append(
        ...
        f"(the hosting decision blocks (internal ref)), so every row below ran from random "
        f"initialisation and quality is pending with a stated cause."
    )
    if tally[STATUS_DIVERGENT] or tally[STATUS_FAIL]:
        out.append("")
        out.append(
            f"Only the {tally[STATUS_PASS]} `PASS` rows meet the acceptance sweep's exit "
...
def exit_code(report: Dict[str, Any]) -> int:
    ...
    DIVERGENT EXITS NON-ZERO ALONGSIDE FAIL. It does not satisfy the acceptance sweep's
