#!/usr/bin/env python3
"""Apply an internal ticket's backbone-only seed contract to the zoo and the dumps.
...
Two subcommands, in the order they must run:
  ...
  ``strip``  — turn the staged full dumps into backbone-only seeds by removing
               ...
               stack (the hosting decision), so stripping is a pure subtraction on bytes
...
_DOC = f"""# (internal ref) — the task head is NOT carried by the hosted seed.
...
def cmd_check(zoo: Path, verdict: Path) -> int:
    ...
    the hosting decision lands ``manifest.json`` and "hosted" is checkable -- see the
    ...
    for key, record in sorted(results.items()):
        ...
        if declared is None:
            ...
            # with the hosting decision. Until then this is reported, not fatal, and the
    ...
    if undeclared:
        ...
        print(
            ...
            "this becomes fatal when the hosting decision lands manifest.json and "
            "'hosted' is checkable:"
        )
