#!/usr/bin/env python3
"""Assert every migrated template that expects a hosted seed HAS one (the hosting decision).
...
That is not hypothetical. the hosting decision found ``faster_rcnn_resnet`` this way,
...
#:
#: (internal ref): the seven DETR seeds retired with the `hf_transformer` family
...
#: (internal ref) — the exemption has to be asked for, and every entry except the
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        "are hosted (the hosting decision).",
    )
    ...
    if manifest is not None and dumps_dir is None and not (names - inventory.retired):
        ...
        # applies — extended past `not names` to cover the state (internal ref)
