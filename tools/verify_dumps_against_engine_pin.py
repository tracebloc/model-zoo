...
``manifest.json`` at all, there are no staged dumps to protect yet, so this
tool says so and exits 0. Pass ``--require-manifest`` to make even that red.

**That is the branch CI takes on every run, and it will not stop being so when
the store exists.** Two preconditions are outstanding, not one:

  * the dumps must be hosted somewhere (the hosting decision); and
  * a manifest carrying a populated ``dumps`` list must reach ``--manifest``.
    The manifest lives in ``backend`` under ``tools/offline_weights`` and is
    keyed ``entries`` — the shape ``tools/sync_zoo_weights.py`` writes and the
    only shape its ``fetch-all`` can consume. Handed to this tool unchanged it
    takes the fail-closed no-``dumps`` branch, so a manifest arriving is
    necessary and not sufficient. Reconciling the two keys is the schema reconciliation.
...
def run_sweep(
    ...
    if not manifest_path.exists():
        ...
        #
        #   * "the moment a manifest lands" implied one could. Nothing in the
        #     verify-dumps job puts a manifest at this path, so the branch is
        #     STRUCTURAL, taken on every run.
        #   * "will verify every dump" is false even then: the only manifest CI
        #     could obtain is the fetch hook's own, keyed 'entries', which takes
        #     the fail-closed branch below instead of a sweep. A manifest landing
        #     is necessary and not sufficient.
        msg = (
            f"no manifest at {manifest_path}: no weight dumps are declared here, "
            "so there is nothing to verify. This branch is STRUCTURAL, not "
            "incidental — no step in verify-dumps-engine-pin.yml puts a manifest "
            "at this path (it lives in backend under tools/offline_weights and "
            "reaches CI as the dump-manifest artifact, which only the "
            "dump-coverage job consumes), so it is taken on every run "
            "(the arming overclaim). TWO things are outstanding before this tool sweeps "
            "anything, not one: the store must exist (the hosting decision), and a "
            "manifest must reach this path carrying a populated 'dumps' list — "
            "the fetch hook's manifest is keyed 'entries' and would take the "
            "fail-closed branch below, until the two schemas are reconciled "
    ...
    if not dumps:
        ...
        #
        # The exit code is the same for two DIFFERENT causes, so the message
        # discriminates them rather than leaving a reader to guess:
        #   * a genuine stub — someone committed a manifest declaring nothing; and
        #   * the schema divergence — a manifest keyed 'entries' (the shape
        #     tools/sync_zoo_weights.py writes, and the only shape CI's fetch hook
        #     can consume) has no 'dumps' list by construction, so it lands here
        #     with dumps already fetched and verified on disk.
        # Reading the second as the first is what makes "populate it" the wrong
        # advice; the fix there is the schema reconciliation.
        entries = manifest.get("entries") or {}
        if entries:
            detail = (
                f"it declares {len(entries)} dump(s) under 'entries' and carries "
                "no 'dumps' list at all. That is the SCHEMA DIVERGENCE between "
                "the fetch hook (tools/sync_zoo_weights.py both writes and reads "
                "'entries') and this verifier ('dumps'), not a stub manifest — "
                "reconciling the two is the schema reconciliation"
            )
        else:
            detail = (
                "its 'dumps' list is missing or empty, and it declares nothing "
                "under 'entries' either — it is a stub"
            )
        print(
            f"FAIL (fail-closed): manifest {manifest_path} exists but {detail}. "
            "It protects nothing. This tool sweeps only a populated 'dumps' "
            "list; removing the manifest returns to the no-op branch above, "
            "which verifies nothing either.",
            file=sys.stderr,
        )
        return 2
