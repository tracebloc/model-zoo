"""Tests for tools/check_dump_coverage.py — the complement gate (the hosting decision).
...
def test_without_an_inventory_the_gate_reports_classification_only(tmp_path):
    ...
    is hosted yet (the hosting decision), rather than red on a state nobody can fix."""
    zoo = _zoo(tmp_path, {"object_detection/fcos": SEED_EXPECTING})
...
# THE CENSUS IS DERIVED, NOT DECLARED.
#
...
# --- a retirement is a decision, not an orphan --------------
#
...
def test_a_retired_dump_no_template_claims_is_not_an_orphan(tmp_path):
    """The state (internal ref) settles: seven kept dumps, zero live templates for
...
def test_a_staging_dir_cannot_launder_an_unlisted_blob_into_a_retired_one(tmp_path):
    ...
    the manifest is an orphan — which is precisely the direction (internal ref)
...
# --------------------------------------------------------------------------
# (internal ref) — the orphan half must STAY armed in CI.
#
...
# --------------------------------------------------------------------------
# (internal ref) — THE TOKEN MUST STAY IN ITS OWN JOB.
#
