"""Guard the guards in ``tools/od_acceptance_sweep.py`` (the acceptance sweep).
...
def test_the_roster_is_not_empty_and_the_alias_is_in_play() -> None:
    ...
    checked nothing — the silent-green shape of (internal ref).
...
def test_the_quality_cause_is_derived_from_the_payload_not_a_template_list() -> None:
    ...
    (internal ref) is fixing from-scratch normalisation
...
def test_the_markdown_report_is_per_template_and_names_what_ran() -> None:
    ...
    report = {
        ...
        "templates": [
            ...
            {
                "template": "red_one",
                "seeding": "scratch (seed declared, not hosted -- the hosting decision)",
                "status": "FAIL",
                "quality": "pending",
    ...
    assert "800x800 (declared 448)" in table, (
        ...
        "figure at an unstated resolution is unreadable"
    )
...
# ---------------------------------------------------------------------------
# The divergence state (the acceptance sweep's third mechanical status)
# ---------------------------------------------------------------------------
