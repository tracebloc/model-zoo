...
def guard_channel_rounding_rounds_up(module) -> None:
    """``_round_channels`` must round UP, the way ``make_divisible`` does.

    This is the assertion ``guard_arch_table_is_live``'s X bullet used to
    claim and could not make. Rebuilding at another scale
    cannot see the rounding direction: every figure in the published arch
    table is a multiple of 64, so every product with a published multiplier
    is already a multiple of 8 and ``ceil`` and ``round`` agree at all five
    scales. The discrimination needs a channel figure the yaml does not
    contain, which is why this guard calls ``_round_channels`` directly
    instead of going through a build.

    It runs at the SHIPPED multiplier — no scale patching — because 0.50
    already separates the two: an ODD figure gives a half-integer product,
    and 17 -> 8.5 -> 8.5/8 = 1.0625 takes ``ceil`` to 2 (16 channels) and
    ``round`` to 1 (8 channels).

    Both directions are asserted, and the sweep asserts a COUNT: a walk that
    happened to visit only figures where the two agree would pass exactly as
    quietly as a correct implementation.
    """
    import math

    ceil_up = []
    for figure in range(1, 200):
        scaled = min(figure, module.MAX_CHANNELS) * module.WIDTH_MULT
        quotient = scaled / module.CHANNEL_DIVISOR
        if math.ceil(quotient) == round(quotient):
            continue
        expected_ceil = max(
            module.CHANNEL_DIVISOR,
            int(math.ceil(quotient) * module.CHANNEL_DIVISOR),
        )
        expected_round = max(
            module.CHANNEL_DIVISOR,
            int(round(quotient) * module.CHANNEL_DIVISOR),
        )
        # Only the figures where the two genuinely disagree can discriminate;
        # a `max(CHANNEL_DIVISOR, ...)` floor can still collapse them.
        if expected_ceil != expected_round:
            ceil_up.append((figure, expected_ceil, expected_round))

    assert len(ceil_up) == 89, (
        f"fixture is degenerate: expected 89 figures in 1..199 to separate "
        f"ceil from round at WIDTH_MULT={module.WIDTH_MULT} and "
        f"CHANNEL_DIVISOR={module.CHANNEL_DIVISOR}, found {len(ceil_up)}. "
        f"With none, this guard cannot fail; the multipliers moved and the "
        f"discriminating set has to be re-derived."
    )

    wrong = [
        (figure, module._round_channels(figure), up, nearest)
        for figure, up, nearest in ceil_up
        if module._round_channels(figure) != up
    ]
    assert not wrong, (
        f"{module.__name__}: _round_channels rounds to NEAREST, not UP, at "
        f"{len(wrong)} of {len(ceil_up)} discriminating figures. First three "
        f"(figure, got, ceil, round): {wrong[:3]}. `make_divisible` rounds up, "
        f"so a nearest-rounding version narrows a stage by 8 channels wherever "
        f"the scaled width lands below a midpoint — and it is INVISIBLE at "
        f"every published scale, because each arch-table figure is a multiple "
        f"of 64 and every product with a published multiplier is already a "
        f"multiple of 8. That is exactly why this guard uses off-table figures "
        f"rather than a rebuild."
    )

    # The named case from the ticket, spelled out so the arithmetic is legible
    # in the failure rather than only in the sweep.
    assert module._round_channels(17) == 16, (
        f"{module.__name__}: _round_channels(17) is "
        f"{module._round_channels(17)}, expected 16 — "
        f"17 * {module.WIDTH_MULT} = 8.5, and 8.5 / 8 = 1.0625 rounds UP to 2, "
        f"giving 16 channels. Nearest rounding gives 1 and 8."
    )
...
def guard_arch_table_is_live(module) -> None:
    ...
    * **X** (width 1.50) is the ONLY scale whose width multiplier is above
      1.00, and that makes it the only witness to two distinct errors:
      a multiplier clamped or saturated at 1.0 — invisible at n/s, which are
      below 1, and at m/l, which are exactly 1.00 (measured: -31,594,016
      parameters at X, survives at every other scale); and the channel cap
      being applied BEFORE the multiplier rather than after, which
      ``_round_channels``'s own docstring calls out as transcribed rather
      than simplified. ``min(c, cap) * mult`` and ``min(c * mult, cap)`` are
      algebraically identical whenever ``mult <= 1``, so n/s/m/l cannot tell
      them apart; at X the second form yields 512 where the first yields 768
      (measured: -27,843,840).

    **X does NOT separate the rounding direction, and this bullet used to
    claim it did**. Measured rather than reasoned about: every
    channel figure in either ``yolo11``'s or ``yolo12``'s arch table
    (``{64, 128, 256, 512, 1024}``) times every published width multiplier is
    already an exact multiple of 8, so ``ceil`` and ``round`` agree at all
    five scales, X included — 50 (figure, multiplier) pairs evaluated, 0
    disagreements. The ``math.ceil -> round`` mutation SURVIVED both this
    guard and ``published_architecture``, so the sentence advertised a
    discrimination the arithmetic makes impossible.

    No rebuild can recover it, because this guard only varies the
    multipliers and the published architecture supplies nothing but
    multiples of 64. The discrimination is real but needs an off-table
    figure, so it lives in ``guard_channel_rounding_rounds_up``, which calls
    ``_round_channels`` directly and has a registered mutation that goes red.
    That is where a ``ceil``-vs-``round`` claim can be true; do not re-add
    one here.
...
GUARDS = {
    ...
    "channel_rounding_up": guard_channel_rounding_rounds_up,
...
MUTATIONS = [
    ...
    (
        # (internal ref): this mutation SURVIVED both arch_table_is_live and
        # published_architecture, which is why the rounding claim moved out of
        # arch_table_is_live's X bullet and into a guard that can see it.
        "channel_width_rounds_to_nearest",
        "        CHANNEL_DIVISOR, int(math.ceil(scaled / CHANNEL_DIVISOR) "
        "* CHANNEL_DIVISOR)",
        "        CHANNEL_DIVISOR, int(round(scaled / CHANNEL_DIVISOR) "
        "* CHANNEL_DIVISOR)",
        "channel_rounding_up",
    ),
    (
