...
#: tracked as (internal ref). Value is ``(norm class name, number of such
...
NORM_FREE_BY_DESIGN = {
    "ssd_vgg16": (
        ...
        "separate one from (internal ref); this file does not settle it."
    ),
}
...
def test_norm_layers_normalise_a_from_scratch_build(path):
    ...
    if probed_backbone is None:
        ...
        assert probed_backbone, (
            ...
            f"(internal ref) failure mode with the norm removed rather than "
    ...
    assert (found_classes, len(leaky)) == ([known[0]], known[1]), (
        ...
        f"  - Changed some other way? (internal ref) needs updating before this "
        f"row does."
    )
