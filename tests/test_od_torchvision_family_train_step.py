...
def _schema_path() -> pathlib.Path:
    ...
    ``hf_transformer`` with (internal ref)), and a test pinning a filename breaks
...
def _read_model_type(path: pathlib.Path) -> str | None:
    ...
    extracting a helper into is being rewritten by (internal ref) right now.
...
, so a template in this family is handed ``[1, C]`` and must
...
def test_family_templates_were_found() -> None:
    ...
    silent-green shape of (internal ref). Also asserts the alias is in play, since
    ...
    the next author to "raise it with an internal ticket's Tier 0". A floor that every
    ...
    (internal ref)). It is now a PARTITION instead, derived from the vendored
...
def test_guard_rejects_a_mask_headed_model() -> None:
    ...
    test fails, and re-homing Mask R-CNN becomes worth revisiting.
    """
    torch = pytest.importorskip("torch", reason="pytorch not installed in this CI job")
    pytest.importorskip("torchvision", reason="torchvision not installed in this CI job")
    ...
    assert "mask" in str(excinfo.value).lower(), (
        ...
        f"assertion; if it stopped requiring masks, see (internal ref)"
    )


def test_guard_rejects_a_head_that_omits_the_background_channel() -> None:
    """The label-range half of the guard, shown going red.
    ...
    # range this file used before (internal ref). It passes — so the red above is
