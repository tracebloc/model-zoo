"""Guards for ``object_detection/pytorch/yolov8_s.py``, each proven able to go
...
``yolov8_s.py`` the backbone, neck, head, assigner and all three losses are
...
* the assigner loses its per-level structure — one stride used for every level,
  or a DFL target computed in pixels instead of cells;
* a "decoupled" head is secretly coupled, or a ``C2f`` is secretly a ``C3``;
...
--------------------------------------
**The eval path is vacuous on a fresh model.** YOLOv8's class prior is
...
``_reference_yolov8_parameters``, which is derived from the published arch
table with nothing from ``model_zoo/`` imported, and anchored to two figures
from outside this repo entirely — see ``_PUBLISHED``.
"""
...
TEMPLATE = OD_PYTORCH / "yolov8_s.py"
...
# from `ultralytics/cfg/models/v8/yolov8.yaml` and `nn/modules/{block,head}.py`,
# with NOTHING under `model_zoo/` imported. And the transcription is itself
# anchored to figures from outside this repo — the Ultralytics model summaries —
# so it cannot drift into agreeing with a wrong template.

#: Published parameter counts, from the Ultralytics model summaries for the
#: 80-class detection models. Two scales, not one, so the anchor pins the WIDTH
#: and DEPTH handling and not just one arithmetic total:
#:
#:   YOLOv8s summary: 11166560 parameters, 11166544 gradients
#:   YOLOv8n summary:  3157200 parameters,  3157184 gradients
#:
...
_PUBLISHED = {
    # (width, depth, max_channels): (published total, published gradients)
    (0.50, 0.33, 1024): (11_166_560, 11_166_544),
    (0.25, 0.33, 1024): (3_157_200, 3_157_184),
}
...
def _reference_yolov8_parameters(
    class_channels, width=0.50, depth=0.33, max_channels=1024, reg_max=16
):
    """YOLOv8 parameter count, derived from the published spec alone.

    ``yolov8.yaml``: a 3x3/s2 stem, four (3x3/s2 downsample + C2f) stages at
    3/6/6/3 blocks, an SPPF on the deepest, a PAN neck of four C2f stages at 3
    blocks, and a Detect head with ``4 * reg_max`` box channels.
    """
    ...
    def bottleneck(channels):
        ...
        # YOLOX's is 1x1 then 3x3 — a ~4x difference per block, and the kind of
        # drift only this comparison catches.
        return _cna(channels, channels, 3) + _cna(channels, channels, 3)

    def c2f(in_ch, out_ch, blocks):
        half = int(out_ch * 0.5)
        return (
            _cna(in_ch, 2 * half, 1)
    ...
    def sppf(channels, repeats=3):
        half = channels // 2
        return _cna(channels, half, 1) + _cna(half * (repeats + 1), channels, 1)

    stem = scale_width(64)
    stages = [
        (scale_width(out_full), scale_depth(blocks_full))
        for out_full, blocks_full in ((128, 3), (256, 6), (512, 6), (1024, 3))
    ]

    total = _cna(3, stem, 3)
    in_ch = stem
    for out_ch, blocks in stages:
        total += _cna(in_ch, out_ch, 3) + c2f(out_ch, out_ch, blocks)
        in_ch = out_ch
    total += sppf(stages[-1][0])

    c3, c4, c5 = stages[1][0], stages[2][0], stages[3][0]
    neck_blocks = scale_depth(3)
    total += c2f(c5 + c4, c4, neck_blocks)
    total += c2f(c4 + c3, c3, neck_blocks)
    total += _cna(c3, c3, 3) + c2f(c3 + c4, c4, neck_blocks)
    total += _cna(c4, c4, 3) + c2f(c4 + c5, c5, neck_blocks)

    box_hidden = max(16, c3 // 4, reg_max * 4)
    cls_hidden = max(c3, min(class_channels, 100))
    for channels in (c3, c4, c5):
...
_REFERENCE_STRUCTURE = {
    ...
    # projects all three levels to a common width. Getting this wrong is a
    # plausible cross-template copy and changes the count too.
    "neck_out": (128, 256, 512),
    "backbone_c2f_blocks": (1, 2, 2, 1),
    "neck_c2f_blocks": (1, 1, 1, 1),
    "box_hidden": 64,
    "cls_hidden": 128,
    "strides": (8, 16, 32),
    "reg_max": 16,
...
def test_the_reference_derivation_matches_the_published_figures() -> None:
    ...
    are pinned so the check covers the width and depth multipliers rather than
    one arithmetic accident, and the DFL gap is asserted explicitly rather than
    hidden in a tolerance.
    """
    for (width, depth, max_channels), (total, gradients) in _PUBLISHED.items():
        assert total - gradients == _DFL_PROJECTION_CONSTANTS, (
            f"published YOLOv8 at width {width}: total {total:,} minus "
            f"gradients {gradients:,} is {total - gradients}, not the "
            f"{_DFL_PROJECTION_CONSTANTS} DFL projection constants. One of the "
            f"two figures is mis-transcribed."
        )
        derived = _reference_yolov8_parameters(
            _PUBLISHED_CLASSES, width=width, depth=depth, max_channels=max_channels
        )
        assert derived == gradients, (
            f"the spec transcription derives {derived:,} parameters at width "
            f"{width} / depth {depth} / {_PUBLISHED_CLASSES} classes, but the "
            f"published summary reports {gradients:,} gradients "
            f"({total:,} total, of which {_DFL_PROJECTION_CONSTANTS} are the "
            f"frozen DFL projection this template does not store). Fix the "
            f"transcription against yolov8.yaml before trusting any comparison "
            f"that uses it."
        )
...
def _backbone_c2f_blocks(model):
    return tuple(len(stage.m) for stage in model.backbone.stages)


def _neck_c2f_blocks(model):
    neck = model.neck
    return tuple(
        len(getattr(neck, name).m)
...
def guard_matches_the_published_architecture(module) -> None:
    ...
    expected = _reference_yolov8_parameters(class_channels)
    model = _build(module, module.output_classes)
    actual = sum(p.numel() for p in model.parameters())

    assert actual == expected, (
        f"{module.__name__}: built model has {actual:,} parameters; YOLOv8-S "
        f"re-derived from its published spec has {expected:,} at the same "
        f"{class_channels} class channels — a difference of "
        f"{actual - expected:+,}. Something in the width, depth, kernel sizes, "
        f"block counts or head shape does not match the design this template "
    ...
    assert _backbone_c2f_blocks(model) == reference["backbone_c2f_blocks"], (
        f"{module.__name__}: backbone C2f stages hold "
        f"{_backbone_c2f_blocks(model)} blocks, published design has "
        f"{reference['backbone_c2f_blocks']} at this depth multiplier"
    )
    assert _neck_c2f_blocks(model) == reference["neck_c2f_blocks"], (
        f"{module.__name__}: neck C2f stages hold {_neck_c2f_blocks(model)} "
        f"blocks, published design has {reference['neck_c2f_blocks']}"
    )
...
#: more — see the block comment above ``_reference_yolov8_parameters`` for the
#: sibling template where exactly such a number was cited as evidence and was
#: wrong. Parameters are asserted against the re-derived published spec in
#: ``guard_matches_the_published_architecture``; what lives here is only what
#: that derivation does not cover.
#:
#: Updating these is legitimate when the architecture changes on purpose; state
#: the intended change in the commit message.
_PINNED_TOTALS = {"buffers": 0, "tensors": 183}
...
def guard_module_tree_size_is_pinned(module) -> None:
    ...
    assert actual == _PINNED_TOTALS, (
        ...
        f"checked against the re-derived published spec in "
...
def guard_no_stateful_normalisation(module) -> None:
    ...
    a bit-exact identity anyway, i.e. it normalises nothing.
    """
    ...
    group_norms = [
        sub for sub in model.modules() if isinstance(sub, nn.GroupNorm)
    ]
    assert group_norms, (
...
def guard_norm_groups_are_derived_from_the_channel_count(module) -> None:
    ...
    ⚠️ THE HONEST STATEMENT FIRST: at the shipped ``WIDTH_MULT = 0.50`` every
    channel count in this tree is a multiple of 32, so a hardcoded 32 would
    work — for this build only. That is exactly the shape of a constant that
    reaches nothing, so this guard does not stop at the shipped build: it
    rebuilds at YOLOv8-N's ``0.25``, where the stem is 16 channels and
    ``GroupNorm(32, 16)`` raises outright, and asserts a sub-32 group count is
    genuinely produced. The width multiplier is a live knob — it is what selects
    the scale — so the derivation is what keeps it usable.
    """
    import torch
    from torch import nn
    ...
    saved = module.WIDTH_MULT
    try:
        module.WIDTH_MULT = 0.25
        try:
            model = _build(module, 3)
        except Exception as error:  # noqa: BLE001 — any build failure is the bug
            raise AssertionError(
                f"{module.__name__}: rebuilding at WIDTH_MULT = 0.25 "
                f"(YOLOv8-N's width) failed with {type(error).__name__}: "
                f"{error}. A hardcoded GroupNorm group count crashes here — the "
                f"stem is 16 channels at this width — which is why the count is "
                f"derived. The width multiplier is a live knob, not decoration."
            ) from error
        ...
        assert any(groups < 32 for groups, _ in pairs), (
            f"fixture is degenerate: every GroupNorm at WIDTH_MULT = 0.25 still "
            f"takes 32 groups ({sorted(pairs)}), so a hardcoded 32 would pass "
            f"this guard too and the derivation is not being exercised"
        )
        for groups, channels in sorted(pairs):
            assert channels % groups == 0, (
                f"{module.__name__}: GroupNorm({groups}, {channels}) at "
                f"WIDTH_MULT = 0.25 does not divide"
            )
        ...
        module.WIDTH_MULT = saved


def guard_c2f_fuses_every_intermediate_block(module) -> None:
    """``C2f`` must concatenate the two split halves **and every intermediate
    block output** — that is the whole difference from YOLOv5/YOLOX's ``C3``,
    which keeps only the final block output and the skip.

    Checked functionally, not from the constructor: a C3-shaped forward that
    repeats its last output to fill the channel budget builds, trains and
    reports identical loss keys. So the expected branch list is reconstructed
    from ``cv1`` and ``m`` and compared against the tensor ``cv2`` is actually
    handed.
    """
    import torch

    blocks = 2
    stage = module.C2f(16, 16, n=blocks, shortcut=False)
    stage.eval()
    assert len(stage.m) == blocks >= 2, (
        f"fixture is degenerate: {len(stage.m)} block(s). At n = 1 the "
        f"'every intermediate' and 'only the last' layouts contain the same "
        f"tensors, so the rule cannot fire"
    )
    assert stage.cv2.conv.in_channels == (2 + blocks) * stage.hidden, (
        f"C2f's fusion conv takes {stage.cv2.conv.in_channels} channels for "
        f"{blocks} blocks at hidden width {stage.hidden}; C2f's shape is "
        f"(2 + n) * hidden = {(2 + blocks) * stage.hidden}. A (2 * hidden) "
        f"input is a C3, not a C2f."
    )
    ...
    assert captured[0].shape == expected.shape, (
        f"C2f hands its fusion conv {tuple(captured[0].shape)}; the C2f branch "
        f"list is {tuple(expected.shape)}"
    )
    assert torch.allclose(captured[0], expected, atol=1e-5), (
        "C2f's fusion conv is not receiving [split_0, split_1, m0(split_1), "
        "m1(m0(split_1))]. The channel count is right and the model trains, so "
        "the likely shape is a C3 padded to the C2f width — the intermediate "
        "block output is being dropped and its capacity wasted."
    )
...
def guard_reg_max_reaches_the_head_and_the_decode(module) -> None:
    ...
    assert module.REG_MAX == 16, (
        f"{module.__name__}: REG_MAX is {module.REG_MAX}; YOLOv8 publishes 16, "
...
.
    ...
    term is capped at 100. If the tower ever became class-count dependent, the
    ...
    assert not uncovered, (
        ...
        f"shape mismatch (internal ref) exists to remove. Re-run "
...
def guard_head_flatten_order_matches_the_anchor_table(module) -> None:
    ...
    for level, (height, width) in enumerate(_ORDERING_SHAPES):
        ...
        head.cls_preds[level] = _select_channel_zero(
            nn, channels, model.num_classes
        )
...
def guard_dfl_loss_interpolates_between_the_two_bracketing_bins(module) -> None:
    ...
    mixed = float(
        module._distribution_focal_loss(even_split(3, 4), half_target)
    )
...
def guard_decode_is_per_image_and_aligned(module) -> None:
    ...
    A freshly built YOLOv8 head predicts around ``sigmoid(-9)`` on every class,
    so a forward pass at initialisation returns nothing but noise and any check
    ...
    results = model._predictions(cls_logits, dist_logits, anchors, [(64, 64), (64, 64)])
    ...
    # Channel 0 is never a positive target -- since (internal ref) the family
...
def guard_declared_image_size_is_the_measured_edge(module) -> None:
    ...
    ``tests/test_od_declared_resolution.py``, which reads the
    ...
    assert (height, width) == (declared, declared), (
        ...
        f"detail it could have had. Build the transform with "
...
def guard_tal_metric_weights_localisation_over_classification(module) -> None:
    ...
    exponents anchor B wins by ~10x, under the swap anchor A wins by ~500x. If
    either stops holding, the fixture — not the rule — has broken.
    """
    import torch
    ...
    assert better == 1, (
        ...
        f"the confidently-classified but badly-boxed one. YOLOv8's alignment "
...
def guard_tal_target_is_the_normalised_alignment_metric(module) -> None:
    ...
    YOLOv8 has **no objectness branch**: the score it ranks by at inference is
...
def guard_tal_breaks_ties_by_iou_not_by_alignment(module) -> None:
    ...
    **by IoU** — the published tie-break, and not the same thing as the
    ...
    assert int(matched_labels[0]) == labels[1], (
        ...
        f"with label {int(matched_labels[0])}; YOLOv8 resolves a multi-claim by "
        f"IoU, so it belongs to label {labels[1]} (IoU {float(ious[1]):.3f} "
        f"against {float(ious[0]):.3f}). Ground truth 0 has the higher "
        f"ALIGNMENT metric ({alignment[0]:.4f} against {alignment[1]:.4f}), so "
        f"both a by-alignment tie-break and no tie-break at all land here."
    )
...
def guard_constructs_with_no_network(module) -> None:
    ...
    def refuse(*_args, **_kwargs):
        raise OSError("network access is blocked by test_yolov8_s")
...
#: smaller. Measured on the shipped template: the loss falls 14.6 -> 1.5 and the
#: top detection reaches score 0.99 at IoU 0.97, so the thresholds below carry
...
def guard_overfits_a_single_object(module) -> None:
    ...
    from the assigner through the three losses to the DFL decode, and it is what
    makes "trains" and "evaluates" claims about this template rather than about
    its return types.
    ...
    ``1 - ious`` in the alignment metric — still overfits this fixture to IoU
    0.94 at score 0.49, comfortably past the thresholds below. The reason is
    that the metric only chooses *which* anchors are positive; the box and DFL
    losses then regress whichever anchors were chosen towards the true box, and
    the inside-the-box prefilter keeps them all on the object. So a wrong
    ranking is not visible end-to-end on one object, and the IoU assertion
    below is NOT evidence that the assigner ranks correctly. That property is
    pinned by ``tal_metric_exponents``, which is where that mutation is
    registered after surviving here.
    """
    import torch
    from torchvision.ops import box_iou
    ...
    assert iou >= 0.35, (
        ...
        f"the alignment metric passes here (measured 0.94), because the box "
        f"loss regresses whichever anchors were chosen towards the true box. "
        f"See tal_metric_exponents."
    )
...
GUARDS = {
    ...
    "c2f_fuses_all_blocks": guard_c2f_fuses_every_intermediate_block,
...
MUTATIONS = [
    (
        "yolox_style_bottleneck_kernel",
        "        self.conv1 = ConvNormAct(in_ch, hidden, 3, stride=1)",
        "        self.conv1 = ConvNormAct(in_ch, hidden, 1, stride=1)",
        "published_architecture",
    ),
    (
        "bottleneck_squeezes_the_branch_again",
        "            Bottleneck(self.hidden, self.hidden, 1.0, shortcut) "
        "for _ in range(n)",
        "            Bottleneck(self.hidden, self.hidden, 0.5, shortcut) "
        "for _ in range(n)",
        "published_architecture",
    ),
    (
        "neck_projects_to_a_common_width",
        "        self.out_channels = (c3, c4, c5)",
        "        self.out_channels = (c3, c3, c3)",
        "published_architecture",
    ),
    (
        "extra_block_per_c2f_stage",
        "for _ in range(n)\n        )",
        "for _ in range(n + 1)\n        )",
        "module_tree_size",
    ),
    ...
    (
        ...
        for block in self.m:
            deep = block(deep)
        return self.cv2(torch.cat(branches + [deep] * len(self.m), dim=1))""",
        "c2f_fuses_all_blocks",
    ),
    (
        "coupled_head",
    ...
    (
        ...
        "        )",
        "        self.head = YOLOv8Head(self.num_classes, self.neck.out_channels)",
        "reg_max_is_live",
    ),
    (
    ...
    # WORST-localised candidate could not learn to box an object. MEASURED: it
    # can. With the inside-the-box prefilter still in place, the box and DFL
    # losses regress whichever anchors were chosen towards the true box, so 200
    # steps on one object still reach IoU 0.94 at score 0.49 — the overfit guard
    # stays green (its thresholds are 0.35 and 0.10) and the mutation SURVIVED
    # it. The metric's direction is a ranking property, so it is pinned by the
    # ranking guard, which catches this outright.
    (
        "alignment_prefers_the_worst_iou",
        "        alignment = scores.pow(TAL_ALPHA) * ious.pow(TAL_BETA)\n",
        "        alignment = scores.pow(TAL_ALPHA) * "
        "(1.0 - ious).pow(TAL_BETA)\n",
        "tal_metric_exponents",
    ),
]
...
_SILENT_MUTATIONS = frozenset(
    {
        ...
        # silent, it raises. YOLOv8's box tower is 64 channels wide and its
        # class tower 128 (`max(16, ch[0] // 4, 4 * reg_max)` against
...
def test_the_template_exists() -> None:
    ...
    assert re.search(
        ...
        f"cell and an external loss.py, and it is frozen."
    )
    assert "yolo" != (
...
def test_silent_mutations_still_train() -> None:
    ...
    assert len(selected) == len(_SILENT_MUTATIONS), (
        f"_SILENT_MUTATIONS names {sorted(_SILENT_MUTATIONS - {e[0] for e in MUTATIONS})}"
        f" which are not in MUTATIONS"
    )
