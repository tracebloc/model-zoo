...
the import-only test stayed green.
"""
...
# Runtime hub-fetch patterns forbidden under model_zoo/ (design note D6).
# The HuggingFace hub is a closed door: a template must build
...
# Local builds (`weights=None`, timm `create_model(..., pretrained=False)`,
...
# downloads a 97.8 MB ResNet-50 on every construction and matches nothing above.
# That sentence sat in the guard meant to catch the defect and told the next
# person the wrong thing. The offline pattern on any builder with a separate
# backbone is `weights=None` AND `weights_backbone=None`, which is a per-callable
# fact about signatures rather than a pattern, and so is enforced by
# `test_zoo_backbone_weights_explicit.py` instead of by this regex.
