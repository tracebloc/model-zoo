...
def guard_declared_image_size_is_the_measured_edge(module) -> None:
    ...
    this check is ``tests/test_od_declared_resolution.py``, which reads the effective resolution off the transform's
...
def guard_rtmdet_centre_prior_survives_float32(module) -> None:
    """The soft centre prior must stay FINITE over every distance a real image
    can reach, AND stay representable alongside the costs it is added to.

    ``10 ** (distance - SOFT_CENTER_RADIUS)`` overflowed float32 to ``inf``
    past 41.53 strides, and that is reachable in ordinary use rather than in
    theory: a full-frame ground truth at stride 8 puts its own corner priors
    ``sqrt((640/2)^2 * 2) / 8 = 56.57`` strides from its centre.

    An ``inf`` here is exactly the failure ``yolox_s``'s
    ``SIMOTA_OUTSIDE_PENALTY`` comment says a *finite* penalty exists to
    avoid, so this is the RTMDet half of
    ``guard_yolox_outside_penalty_survives_float32`` and is deliberately
    structured the same way. The two templates had the same class of bug from
    opposite directions — YOLOX's penalty was finite but above float32's
    resolution, RTMDet's prior overflowed outright — and both land on
    selection by index order.

    Four assertions, because the two constraints pull in opposite directions
    and the shipped code satisfied only one of them:

    1. finite across the whole reachable distance range, DERIVED from
       ``image_size`` and the finest stride rather than pinned, and evaluated
       through the template's own expression — not a re-implementation of it;
    2. the ceiling exceeds the largest cost the rest of the matrix can attain,
       so a saturated far prior never outranks an unsaturated near one —
       derived from ``_EPS``, the IoU weight and the class count;
    3. adding a small real cost to the ceiling is still observable in float32.
       This is the side that rules out simply clamping higher: 1e7 is finite
       and reachable, and its ULP of 1.0 erases the cost deltas the prior is
       added to, which is ``inf`` with extra steps;
    4. behaviourally: with *both* candidates past the old overflow boundary,
       the better-IoU one still wins.

    Assertion 4 is the one that makes the defect concrete. The overflow is
    otherwise **silent** — ``model(images, targets)`` returns a finite loss
    dict over an all-``inf`` cost matrix, so the train-step suite stays green
    and only the choice of anchor is wrong.

    Note the semantic consequence the fix carries, so it is recorded next to
    the guard that enforces it: the prior saturates at 8.0 strides, so beyond
    that candidates rank by IoU and classification cost rather than by
    distance. ``guard_rtmdet_soft_centre_prior_beats_a_better_iou`` and
    ``guard_rtmdet_centre_prior_scales_with_stride`` both work entirely
    inside 8 strides and are unaffected.
    """
    import torch
    from torchvision.ops import box_iou

    model = _build(module, 2)
    ceiling = module.SOFT_CENTER_PRIOR_CEILING
    finest = float(min(module.STRIDES))

    # (1) finite over the full reachable range. The far corner of a full-frame
    # ground truth against its own centre, at the finest stride, is the
    # worst case this template can produce — derived, not pinned.
    edge = float(module.image_size)
    reach = math.sqrt((edge / 2) ** 2 + (edge / 2) ** 2) / finest
    boundary = math.log10(torch.finfo(torch.float32).max) + module.SOFT_CENTER_RADIUS
    assert reach > boundary, (
        f"fixture is degenerate: a full-frame ground truth at stride "
        f"{finest:.0f} reaches {reach:.2f} strides, which no longer passes "
        f"float32's overflow boundary of {boundary:.2f} — this guard would "
        f"pass on the unclamped expression too"
    )
    # Walk the range rather than probing one point, and assert the COUNT: a
    # sweep that evaluated nothing would pass as quietly as one that found
    # nothing wrong.
    sweep = torch.linspace(0.0, reach, 200, dtype=torch.float32)
    assert sweep.numel() == 200, f"sweep built {sweep.numel()} distances, expected 200"
    priors = torch.stack(
        (torch.zeros_like(sweep), sweep * finest, torch.full_like(sweep, finest)),
        dim=1,
    )
    gt_boxes = torch.tensor([[-edge, -edge, edge, edge]])
    gt_labels = torch.tensor([0], dtype=torch.int64)
    boxes = torch.zeros(sweep.numel(), 4)
    boxes[:, 2:] = edge

    # Read the prior out of the TEMPLATE's own expression rather than
    # recomputing it here. Recomputing is what makes a numeric guard a
    # tautology: a copy of the fixed formula in the test is finite whatever
    # the template does, and this assertion would then pass on the overflowing
    # code it exists to reject.
    captured = []
    real_pow = torch.pow

    def _spy(base, exponent, *args, **kwargs):
        result = real_pow(base, exponent, *args, **kwargs)
        if torch.is_tensor(result) and result.dim() == 2 and result.is_floating_point():
            captured.append(result.detach().clone())
        return result

    torch.pow = _spy
    try:
        _rtmdet_assign(model, gt_boxes, gt_labels, boxes, priors)
    finally:
        torch.pow = real_pow

    assert len(captured) == 1, (
        f"fixture is degenerate: expected to observe exactly ONE torch.pow "
        f"producing the (candidates, gts) soft centre prior inside assign(), "
        f"observed {len(captured)}. The guard is reading the template's own "
        f"expression, so it has to know which call that is."
    )
    prior = captured[0].flatten()
    assert prior.numel() == sweep.numel(), (
        f"fixture is degenerate: the prior covers {prior.numel()} candidates "
        f"but {sweep.numel()} distances were swept — some prior fell outside "
        f"the ground truth and never became a candidate"
    )
    assert prior.dtype == torch.float32, (
        f"fixture is degenerate: the prior is {prior.dtype}, not float32, so "
        f"this guard is not measuring the dtype the head actually emits"
    )
    infinite = int((~torch.isfinite(prior)).sum())
    assert infinite == 0, (
        f"rtmdet_s: the soft centre prior is non-finite at {infinite} of "
        f"{sweep.numel()} distances swept over 0..{reach:.2f} strides — the "
        f"range a full-frame ground truth at stride {finest:.0f} actually "
        f"reaches. float32 overflows past {boundary:.2f} strides, and an inf "
        f"in the cost matrix ties every far candidate, so topk selects by "
        f"index order instead of by IoU."
    )
    # Monotonic over that range: a nearer prior must never cost more than a
    # farther one, or the cost is not usable for ranking at all.
    regressions = int((prior[1:] < prior[:-1]).sum())
    assert regressions == 0, (
        f"rtmdet_s: the soft centre prior decreases with distance at "
        f"{regressions} of {sweep.numel() - 1} steps across "
        f"0..{reach:.2f} strides, so a nearer prior can cost more than a "
        f"farther one and the cost is no longer usable for ranking."
    )

    # (2) dominates every attainable real cost, re-derived from the template's
    # own constants rather than pinned to a number someone read once.
    worst_cls = model.num_classes * -math.log(module._EPS)
    worst_iou = module.ASSIGNER_IOU_WEIGHT * -math.log(module._EPS)
    attainable = worst_cls + worst_iou
    assert ceiling > attainable, (
        f"rtmdet_s: SOFT_CENTER_PRIOR_CEILING is {ceiling:,.0f} but the rest "
        f"of the cost matrix can reach {attainable:,.1f} (quality-focal BCE "
        f"{worst_cls:,.1f} at {model.num_classes} channels plus IoU "
        f"{worst_iou:,.1f}, both at the _EPS clamp). A saturated far prior "
        f"can then outrank a near one."
    )

    # (3) still resolvable in float32 next to a realistic cost delta. 0.01 is
    # about the cost difference a 0.01 IoU change makes near IoU 0.5
    # (3 * (log(0.51) - log(0.50)) = 0.059), the same criterion the YOLOX twin
    # derives, and it is what rules out clamping at a larger value.
    smallest = 0.01
    saturated = torch.tensor([0.0, smallest], dtype=torch.float32) + ceiling
    as_f32 = torch.tensor(ceiling, dtype=torch.float32)
    ulp = float(as_f32.nextafter(torch.tensor(float("inf"))) - as_f32)
    assert float(saturated[1]) != float(saturated[0]), (
        f"rtmdet_s: SOFT_CENTER_PRIOR_CEILING is {ceiling:,.0f}, whose float32 "
        f"ULP is {ulp:g} — a real cost difference of {smallest} vanishes when "
        f"added to it. The prior is capped instead of left to overflow "
        f"precisely so the matrix stays comparable; above float32's "
        f"resolution a finite cap is inf with extra steps, and topk falls "
        f"back to index order."
    )

    # (4) behaviour, with BOTH candidates past the old overflow boundary.
    priors = torch.tensor([[8.0, 8.0, finest], [16.0, 16.0, finest]])
    gt_boxes = torch.tensor([[0.0, 0.0, edge, edge]])
    gt_labels = torch.tensor([0], dtype=torch.int64)
    # Index 0 is the WORSE box (IoU 0.3025) and also the FARTHER from the GT
    # centre; index 1 is better on both counts. Order matters: topk breaks an
    # exact tie towards the lower index, so the better candidate has to sit
    # second for a collapsed matrix to pick wrongly.
    boxes = torch.tensor(
        [[0.0, 0.0, edge * 0.55, edge * 0.55], [0.0, 0.0, edge * 0.95, edge * 0.95]]
    )
    distances = (
        (priors[:, :2] - torch.tensor([edge / 2, edge / 2])).pow(2).sum(dim=-1).sqrt()
        / finest
    )
    assert bool((distances > boundary).all()), (
        f"fixture is degenerate: both candidates must sit past float32's "
        f"overflow boundary of {boundary:.2f} strides for this fixture to see "
        f"the defect — got {distances.tolist()}"
    )
    ious = box_iou(boxes, gt_boxes).flatten()
    assert float(ious[1]) > float(ious[0]) and float(distances[1]) < float(
        distances[0]
    ), (
        f"fixture is degenerate: candidate 1 must be both better-IoU and "
        f"nearer the centre, so no term in the cost prefers candidate 0 — got "
        f"IoUs {ious.tolist()} at distances {distances.tolist()}"
    )

    positive_mask, _, matched_ious = _rtmdet_assign(
        model, gt_boxes, gt_labels, boxes, priors
    )
    assert positive_mask.tolist() == [False, True], (
        f"rtmdet_s: with both candidates {distances[0]:.1f} and "
        f"{distances[1]:.1f} strides from the ground-truth centre — past the "
        f"{boundary:.2f} where the unclamped prior overflows — the assigner "
        f"selected {positive_mask.nonzero().flatten().tolist()} rather than "
        f"candidate 1, whose IoU is {float(ious[1]):.4f} against "
        f"{float(ious[0]):.4f}. Both priors become inf without the exponent "
        f"clamp, the costs tie exactly, and topk falls to index order — so a "
        f"large object gets an arbitrary prior with every other guard green "
        f"and the loss finite."
    )
    assert float(matched_ious[0]) == pytest.approx(float(ious[1]), abs=1e-6)
...
RTMDET_GUARDS = {
    ...
    "centre_prior_float32": guard_rtmdet_centre_prior_survives_float32,
...
MUTATIONS = [
    ...
    # Two mutations for the float32 guard, one per side of its two-sided
    # constraint — mirroring the YOLOX twin, where a value satisfying only one
    # side is exactly what shipped.
    (
        # The pre-fix expression, restored verbatim: overflows to inf
        # past 41.53 strides, which a full-frame GT at stride 8 reaches.
        "rtmdet/unclamped_centre_prior",
        RTMDET_PATH,
        "        soft_centre_prior = torch.pow(\n"
        "            10.0,\n"
        "            (distance - SOFT_CENTER_RADIUS).clamp("
        "max=SOFT_CENTER_PRIOR_MAX_EXPONENT),\n"
        "        )",
        "        soft_centre_prior = torch.pow(10.0, distance - SOFT_CENTER_RADIUS)",
        ("rtmdet", "centre_prior_float32"),
    ),
    (
        # Finite and monotonic, but clamped so high that float32's ULP (1.0 at
        # 1e7) erases the cost deltas the prior is added to — inf with extra
        # steps, and the reason the ceiling is not simply raised to keep more
        # of the original distance ordering.
        "rtmdet/coarse_centre_prior_ceiling",
        RTMDET_PATH,
        "SOFT_CENTER_PRIOR_CEILING = 1.0e5",
        "SOFT_CENTER_PRIOR_CEILING = 1.0e7",
        ("rtmdet", "centre_prior_float32"),
    ),
    (
