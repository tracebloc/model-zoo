...
SCHEMA v2 — the DETR templates are GONE
-------------------------------------------------------
...
def test_od_templates_declaring_model_type_were_found() -> None:
    ...
    an internal ticket's mask_rcnn deletion, with a comment telling the next author to
    raise it again for an internal ticket's roster. A literal every roster PR edits is
    a serialisation point — the same cost the census literal had, measured on
    (internal ref) — and a floor that trails the tree is also a guard that has
    stopped guarding.
...
def test_od_model_type_resolves_in_engine_registry(path: pathlib.Path) -> None:
    ...
    if normalized in NOT_ACCEPTED_EXAMPLES:
        hint = (
            ...
            "DETR family was retired in (internal ref) and is itself rejected."
        )
    assert normalized in ACCEPTED, (
        f"{path.relative_to(ROOT)}: model_type {model_type!r} is not in the "
        f"engine's accepted OD vocabulary {sorted(ACCEPTED)}{hint}"
    )
