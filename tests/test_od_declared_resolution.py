...
The before/after mAP measurement (internal ref) originally wanted was NOT gated
on: it needs seeded weights from the blocked the hosting decision, both arms score ~0
...
the acceptance sweep once seeds exist.
...
-------------------------------------------------------------------
That is the hole this file was rewritten to close (recorded on (internal ref)),
...
PUBLISHED_RESOLUTION: dict[str, tuple[int, str, str]] = {
    ...
    # --- this one; recorded on (internal ref), not fixed here.
...
def _other_family_values() -> frozenset[str]:
    ...
    below assert COVERAGE without a floor to recompute.
    """
    schema = json.loads(_schema_path().read_text(encoding="utf-8"))
    values: set[str] = set()
...
def test_family_templates_were_found():
    ...
    # the census literal had. It is a PARTITION now: the schema
...
def test_declared_image_size_is_the_resolution_the_model_runs_at(path):
    ...
    if known is None:
        assert declared == effective, (
            ...
            f"{declared} if that is really the intent. See (internal ref)."
        )
        return
    ...
    assert (declared, effective) == known, (
        ...
        f"  - If it changed some other way, (internal ref) needs updating before "
        f"this row does."
    )
...
def test_known_mismatches_are_all_still_mismatched():
    ...
    assert not stale, (
        ...
        f"list decay into folklore. See (internal ref)."
    )
...
def test_the_known_mismatch_list_only_ever_shrinks():
    ...
    **The ratchet is now AT ITS FLOOR** — empty, pinned at 0 (model-zoo#265).
    ...
    assert MAX_KNOWN_MISMATCHES == 0, (
        ...
        f"reached when (internal ref) fixed the last three templates "
...
def test_every_od_template_has_a_published_resolution_row():
    ...
    assert not missing, (
        ...
        f"the tautology this table exists to remove."
    )
    ...
    **empty** (model-zoo#265), so this test exempts nothing: every template's
    ...
    if known is not None:
        ...
        assert declared == recorded_declared, (
            ...
            f"  - Changed to something else? (internal ref) needs updating "
    ...
    assert declared == published, (
        ...
        f"tautology (internal ref) was reopened about."
    )
...
def test_the_engine_anchor_reads_the_field_not_the_prose():
    ...
    chatty = (
        "Grid-tensor output + external loss, fixed 448 input. See (internal ref); "
...
def test_the_two_readers_are_independent_and_discriminate(tmp_path):
    ...
    go empty together and it would pass on nothing."""
