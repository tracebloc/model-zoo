...
- **Freezing BN is not an equivalent alternative to that, and this bullet used to offer it as one.** Freezing answers the averaging problem and creates a worse one whenever the template builds from scratch: `FrozenBatchNorm2d` at construction holds `weight=1`, `bias=0`, `running_mean=0`, `running_var=1`, so it computes `(x - 0) / sqrt(1 + eps)` — the input, unchanged. Those buffers are only meaningful once a pretrained checkpoint loads real statistics into them, which is what the layer is *for*; on a `weights=None` backbone there is nothing to freeze and the layer is a no-op. Twelve shipped OD templates took this advice and trained with **no backbone normalisation at all** (backend#3093; activations reach sigma ~= 24 at the ROI head against ~= 3 with a live BN, while the loss stays finite and decreasing). All twelve moved to GroupNorm in model-zoo#262. Freeze BN only where a seed is guaranteed to have loaded real statistics first — and since every template in this repo builds `weights=None` and seeds are still blocked on backend#2659, that is nowhere today. `tests/test_od_norm_layers_normalise.py` enforces this for the object-detection roster, in two halves: **(a)** every norm module must normalise a from-scratch build (a constant-shift leak probe, not an `allclose` identity check, which passes `FrozenBatchNorm2d(eps=1e-3)`), and **(b)** the BACKBONE must carry a norm module of its own, because a detector head's own GroupNorms satisfy (a) on their own and would hide a trunk that lost every norm. Its `NON_NORMALISING` known-bad list is now **empty** and `MAX_NON_NORMALISING` is **0** — so there is no known-bad template left, and the list survives only so that the cheapest way to green a newly-broken template is still refused by the ratchet. **Do not add a row to it**; a norm that does not normalise in a new template is that template's bug. What the guard does NOT catch, measured: a norm module that is constructed but never applied, and a partial removal that leaves at least one backbone norm behind. Its own docstring carries both.

  **Not fixed and not guarded: the two `keypoint_detection` templates that build the same `resnet50(weights=None, norm_layer=misc_nn_ops.FrozenBatchNorm2d)` line** (`faster_rcnn_sppe.py`, `keypoint_rcnn.py`) — so it is fourteen templates, not twelve. The guard is driven by the object-detection directory scan and cannot see them; a keypoint sibling guard is the follow-up.
- **FrozenBN -> GroupNorm is not the same arithmetic as BN -> GroupNorm, and mixing them up misreports a fix.** Frozen BN holds `weight`/`bias` as **buffers**; live BN and GroupNorm hold them as **parameters**. So `BatchNorm2d -> GroupNorm` (model-zoo#237, yolox_s/rtmdet_s) leaves the parameter count identical and drops the running-statistic buffers, while `FrozenBatchNorm2d -> GroupNorm` (model-zoo#262, the twelve OD templates) **adds** 2 parameters per normalised channel and drops 4 buffers per channel — on ResNet-50's 26,560 normalised channels that is +53,120 parameters and -106,240 buffer elements per template. Any published-parameter-count anchor has to be recomputed for the second migration, not carried over from the first.
