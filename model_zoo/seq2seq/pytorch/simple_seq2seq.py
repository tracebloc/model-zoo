...
category = "seq2seq"
model_type = ""
tokenizer_file = "seq2seq_tokenizer.json"
# 4, NOT 16. The platform's built-in training envelope is cpu=1,memory=2Gi
# (the jobs-manager default when the installer sets no explicit training
# resources; the 3Gi figure in the envelope contract is the overhead reserved
# for the platform's own pods, SUBTRACTED to size the envelope, not the
# envelope). Measured on one training step (forward, cross-entropy over the
# 30522-token logits, backward, Adam) at sequence_length 128, before the
# training runtime's own footprint: batch 16 peaks at ~2.7 GB of process
# memory, batch 8 at ~1.8 GB, batch 4 at ~1.3 GB. At 16 the pod was OOM-killed
# on a CI edge sized to a 3Gi envelope (the only in-envelope observation on
# record -- it also bounds the training runtime's own footprint at > ~0.5 GB,
# since ~2.7 GB alone would have fitted 3Gi). Against the 2Gi default, 4 is the
# only value in that table that fits at all, and that "fits" is DERIVED, not
# observed: 1.3 GB plus a runtime footprint known only to exceed ~0.5 GB leaves
# a margin of a few hundred MB, and no batch-4 step has yet been measured inside
# a 2Gi pod (the CI row that proves this template runs at 3Gi). Batch buys
# roughly 0.5 GB per halving down to a ~0.8 GB floor of weights, gradients and
# Adam moments that no batch size removes, so batch 2 is NOT a fallback if 4 is
# killed at 2Gi -- a tighter ceiling needs a smaller model (fewer layers, a
# smaller d_model), not a smaller batch. The SDK reads this literal and a
# user cannot override it, so every run now takes 4x the optimizer steps per
# epoch at the same learning rate (which stays the user's knob); convergence
# differs from the batch-16 runs this replaces.
batch_size = 4
sequence_length = 128
vocab_size = 30522
license = "Apache-2.0"
