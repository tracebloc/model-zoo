...
# looked at. `direct` is an accepted choice ("Direct Regression Model") and is
# the one that MATCHES THIS MODEL: the engine's keypoint strategy routes
# `rcnn` and `heatmap` to their own step paths and everything else to
# `_run_direct_step`, and `_VitPoseWrapper.forward` returns an
# `(N, K, 3)` tensor of `[x, y, confidence]` in pixel space.
#
# ⚠️ NOT `heatmap`, and this is the trap worth naming: the wrapped
# `VitPoseForPoseEstimation` DOES produce heatmaps internally, but the
# wrapper soft-argmaxes them to coordinates before returning, so the engine
# never sees a heatmap.
# `heatmap` would route to `_run_heatmap_step`, which expects heatmap tensors
# this template does not emit.
#
# ⚠️ NOT `""` either. An empty declaration is not rejected by the backend
# (the field is `blank=True` and the serializer coerces falsy -> `default`),
# but the engine's `_infer_model_type` falls through an empty value to a
# final `RCNN_FAMILY` fallback -- which would mis-route this coordinate
# regressor onto the R-CNN path.
model_type = "direct"
main_method = "MyModel"
license = "Apache-2.0"
image_size = 256
