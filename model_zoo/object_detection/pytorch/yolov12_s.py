...
(internal ref) and the three ``yolov8_s`` / ``yolov9_s`` / ``yolov10_s``
...
templates, one of which — ``yolo_v8/`` — is a YOLOv8 backbone
...
while their builders resized to ``min_size=800``: the declared
...
class AreaAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # (an internal ticket established this for `relative_position_mlm.py`;
