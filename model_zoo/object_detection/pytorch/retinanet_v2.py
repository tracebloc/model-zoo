...
upload the matched ``retinanet_v2_weights.pkl`` sitting next to this file via
``weights=True``, and the platform loads it after ``MyModel()`` has built
this architecture::

    user.upload_model("retinanet_v2", weights=True)

The seed carries the BACKBONE ALONE (internal ref). The keys under
