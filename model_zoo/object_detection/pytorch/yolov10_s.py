...
templates, and nothing here touches them.
...
hardware that has them. (internal ref) established that conversion for
...
-------------------------------------
Since (internal ref) the family handler owns the translation: it shifts dataset
...
``running_mean=0``, ``running_var=1``, verified in (internal ref)), so freezing it
...
# three — the exact shape mismatch (internal ref) exists to remove.
...
class ConvNormAct(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, ksize=1, stride=1, groups=1, act=True):
        ...
        # bit-exact identity, i.e. no normalisation at all.
...
class Attention(nn.Module):
    ...
    * ATTN_RATIO)`` equals ``dim * ATTN_RATIO`` for any head count that divides
    ...
    def forward(self, x):
        ...
        # (an internal ticket established this for `relative_position_mlm.py`; this is
...
class YOLOv10S(nn.Module):
    ...
    def __init__(self, num_classes=output_classes, input_size=image_size):
        ...
        # reserves. Since (internal ref) targets arrive in model space [1, C], so
