...
. Measured off the built model, not trusted from here:
...
As of (internal ref) the family handler hands this model label space ``[1, C]``:
...
#: Ceiling on the soft centre prior ``10 ** (d / stride - SOFT_CENTER_RADIUS)``,
#: applied by clamping the EXPONENT. Two-sided, and both sides are
#: load-bearing — the same two constraints, derived the same way, as
#: ``yolox_s.py``'s ``SIMOTA_OUTSIDE_PENALTY``:
#:
#: * large enough that a far prior never outranks a near one — so it must
#:   exceed the largest cost the rest of this matrix can attain, which is
#:   ``num_classes * -log(_EPS)`` from the quality-focal BCE plus
#:   ``ASSIGNER_IOU_WEIGHT * -log(_EPS)`` from the IoU term (414.5 at this
#:   template's 12 classes);
#: * small enough that ``ceiling + cost`` is still *representable* in float32,
#:   because a prior above the resolution of the numbers it is added to erases
#:   them, and selection falls to index order.
#:
#: Unclamped this expression **overflowed to inf**. float32's
#: max is 3.4028e38, so the boundary is ``d / stride > 41.53`` — and it is
#: reachable in ordinary use, not theoretically: a full-frame ground truth at
#: stride 8 puts its own corner priors 56.57 strides from its centre
#: (``sqrt(320^2 + 320^2) / 8``), so a 640x640 GT overflows by 15 strides.
#: An ``inf`` here is precisely the failure yolox_s's comment says a FINITE
#: penalty exists to avoid: two far priors both become ``inf``, the cost
#: matrix ties exactly, and ``topk`` returns the lower index. Measured on the
#: unfixed template, ``assign`` picked an IoU-0.3025 prior over an IoU-0.9025
#: one that was also nearer the centre, purely because it came first.
#:
#: 1e5 is the value, and it is forced rather than chosen: it is the largest
#: power of ten whose float32 ULP (0.0078125) still resolves a 0.01 cost
#: delta — roughly the cost of a 0.01 IoU change near IoU 0.5 — which is the
#: identical criterion, and the identical resulting value, that yolox_s
#: derived for its own penalty. 1e6's ULP is 0.0625 and already fails it.
#: 1e5 also clears the attainable cost ceiling above by 241x.
#:
#: The clamp saturates the prior at ``SOFT_CENTER_RADIUS + 5.0`` = 8.0
#: strides, and that is a REAL CHANGE IN ASSIGNER SEMANTICS past that
#: distance, stated here rather than left to be discovered: beyond 8 strides
#: the prior no longer separates candidates, so they rank by IoU and
#: classification cost instead of by distance. Inside 8 strides the values
#: are bit-for-bit what they were. It is a small change in practice because
#: float32 had already
#: taken the other half away — beyond 8.42 strides the *unfixed* prior
#: exceeded 2.62e5, whose ULP erases a 0.01 cost delta, so the unfixed
#: assigner already ranked far candidates by distance ALONE. Both forms
#: discard one of the two terms out there; this one keeps the term the cost
#: matrix exists to compare, and keeps it finite.
#: ``guard_rtmdet_centre_prior_survives_float32`` asserts every side of this
#: rather than trusting the comment.
SOFT_CENTER_PRIOR_CEILING = 1.0e5
SOFT_CENTER_PRIOR_MAX_EXPONENT = math.log10(SOFT_CENTER_PRIOR_CEILING)
...
class RTMDetS(nn.Module):
    ...
    def assign(self, gt_boxes, gt_labels, boxes, cls_logits, priors):
        ...
        2. the cost adds a *soft* centre prior, ``10 ** (d / stride - 3)``,
           ...
           The exponent is clamped at ``SOFT_CENTER_PRIOR_MAX_EXPONENT``: the
           unclamped form overflows float32 to ``inf`` past 41.53 strides,
           which a full-frame GT at stride 8 reaches, and an
           ``inf`` ties the cost matrix so ``topk`` selects by index order;
        ...
        # Clamp the EXPONENT, not the result: past 41.53 strides this
        # overflows float32 to inf, and a full-frame GT at stride 8 reaches
        # 56.57. See SOFT_CENTER_PRIOR_CEILING for why the
        # ceiling is 1e5 and what it costs past 8 strides.
        soft_centre_prior = torch.pow(
            10.0,
            (distance - SOFT_CENTER_RADIUS).clamp(max=SOFT_CENTER_PRIOR_MAX_EXPONENT),
        )
        ...
        # centre prior does the work — it passes the attainable cls+IoU cost
        # ceiling at about 5.6 strides from a GT centre, and saturates at
        # 1e5 from 8.0 strides out (SOFT_CENTER_PRIOR_CEILING) — and that is
        # precisely why the stride column must be real per level.
        # This comment used to read "at ten strides it is 1e7"; that value is
        # no longer reachable, and 1e7's float32 ULP is 1.0, which would erase
        # the very cost deltas the prior is being added to.
    ...
    def _predictions(self, cls_logits, boxes, image_sizes):
        ...
        for image_boxes, class_scores, (height, width) in zip(
            ...
            # label directly. Since (internal ref) the family handler hands this
