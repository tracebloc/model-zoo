...
``faster_rcnn_resnet.py``: with no weights requested the builder unfreezes all
five backbone stages instead of the last three, and it picks the backbone norm
off the same flag. Building the backbone directly keeps the three trainable
stages and the P6/P7 pyramid under explicit control. The norm is no longer the
...
real statistics into them, and NO OD seed is hosted (an internal ticket is blocked
on the store decision in the hosting decision). So every run of this template to date
...
def _group_norm(channels):
    ...
    The backbone norm for a from-scratch build. This template
...
def MyModel(num_classes=output_classes):
    ...
    # No download: GroupNorm backbone (an internal ticket -- frozen BN is an
    # identity from scratch), FPN over the last 3 stages with extra P6/P7
    # levels, and the stock 91-class COCO head (replaced below, as before).
